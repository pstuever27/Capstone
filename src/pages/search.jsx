...
function Search() {
  ...
  async function blockFromQueue(){
    if(songChoice != null){
      ...
      addBlock('add-block', cookie.get('roomCode'), `${songChoice.name} - ${songChoice.artists.map((_artist) => _artist.name).join(", ")}`, songChoice.uri);
      setSongChoice( null ); 
      setNotifMessage("Added to block list");
